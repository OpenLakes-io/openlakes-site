...
Purpose-built console that exposes the entire OpenLakes lakehouse through a single ingress endpoint. Engineers authenticate once (SSO → Console) and receive scoped tokens for downstream services (Airbyte, Airflow, dbt, Trino, OpenMetadata, Superset, MinIO, StarRocks). Everything else remains ClusterIP/private.
...
2. **Ingest** – Guided Airbyte connection builder: source creds from Vault, choose MinIO bucket + Iceberg namespace, auto-generate Airflow DAG for scheduling.
...
2. **Pipelines**
   - Card per pipeline (Airbyte connection + Airflow orchestration). Pause/resume, inspect logs, trigger backfill.
3. **dbt Studio**
...
1. **Foundations**
   ...
   - Build backend service with connectors to Airbyte, Airflow, Trino, OpenMetadata, Helm.
2. **Workflow MVP**
   - Ingestion wizard (Airbyte + Airflow) with secret vault integration.
